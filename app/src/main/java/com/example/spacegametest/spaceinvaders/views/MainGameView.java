...
import com.example.spacegametest.spaceinvaders.views.Ally;
import com.example.spacegametest.spaceinvaders.views.Enemy;
import com.example.spacegametest.spaceinvaders.views.Explosion;
import com.example.spacegametest.spaceinvaders.views.Player;
import com.example.spacegametest.spaceinvaders.views.StarAnim;
import com.example.spacegametest.spaceinvaders.views.activities.MainActivity;
...
public class MainGameView extends SurfaceView implements Runnable {
    ...
    private ArrayList<StarAnim> starAnims = new
            ArrayList<StarAnim>();
    ...
    public MainGameView(Context context, int screenX, int screenY) {
        super(context);
        ...
        for (int i = 0; i < starNums; i++) {
            StarAnim s = new StarAnim(screenX, screenY);
            starAnims.add(s);
        }
        ...
        ally = new Ally(context, screenX, screenY);
    ...
    private void update() {
        ...
        enemies.update(player.getSpeed());
                ...
                if (Rect.intersects(player.getDetectCollision(), enemies.getDetectCollision())) {
                    ...
                    explosion.setX(enemies.getX());
                    explosion.setY(enemies.getY());
        ...
        ally.update(player.getSpeed());
                //checking for a collision between player and a friend
                if(Rect.intersects(player.getDetectCollision(), ally.getDetectCollision())){

                    //displaying the boom at the collision
                    explosion.setX(ally.getX());
                    explosion.setY(ally.getY());
                    //setting playing false to stop the game
                    playing = false;
